...
- [1. 協作角色與權限](#1-協作角色與權限-collaboration-roles--permissions)
  - [1.1 工程師角色 (Engineer Role)](#11-工程師角色-engineer-role)
...
#### 1.1.2 開發流程與版本控制
- **`.gitignore` 檔案控管**：請務必在專案根目錄建立並維護 `.gitignore` 檔案，以確保倉儲的乾淨。**嚴禁**提交 IDE 設定檔、本機環境變數檔 (`.env`)、專案依賴套件 (`node_modules/`)等非交付必要檔案。
  > **提示**：您可以使用 [gitignore.io](https://www.toptal.com/developers/gitignore) 等線上工具快速產生推薦的設定檔。

- **檔案大小限制**：單一檔案請勿超過 100MB，並禁止使用 **Git Large File Storage (Git LFS)**。
...
- **本地端職責**：開發夥伴應在自身的開發環境中完成所有單元測試 (Unit Testing) 與最終的程式碼打包 (Build/Package) 作業。

- **提交至測試環境**：當功能開發完成，且需要交付給業務承辦人進行測試時，請遵循以下步驟：
  1. 將本地打包後產生的交付檔案至 develop 分支 dist 資料夾底下。
  2. 此動作將觸發 CI/CD 流程，將您提交的交付檔案部署至測試環境 (Testing)。

- **環境驗收流程**：
  1. 部署至測試環境後，由業務承辦人與資訊承辦人共同進行功能與規格驗收。
  2. 確認所有功能皆符合需求且無誤後，方可安排將 develop 分支的變更合併至 main 分支，以部署至正式環境 (Production)。

> **重要提示**：本倉儲僅接受打包後的交付檔案。請勿提交開發依賴等非交付必要檔案 (例如 src/, node_modules/, .vscode/) ，以確保倉儲中只包含最終的部署檔案。

#### 1.1.4 CI/CD 與部署說明

- **職責劃分**：我方負責準備 AWS 環境與權限串接； CI/CD 腳本管理，如有修改需求，請聯繫資訊承辦人。

- **部署流程**：
  - `main` 分支的變更部署至**正式環境**
  - `develop` 分支的變更部署至**測試環境**

- **安全須知**：廠商無需處理任何 AWS 金鑰 (Access Keys)，所有權限將透過 GitHub 與 AWS 的信任關係自動授予。

### 1.2 專案經理角色 (PM Role) - 議題管理者

#### 1.2.1 存取方式：GitHub 帳號
- **權限等級**：PM 將被授予 Triage 權限，專注於議題管理，無法推送或修改程式碼。
...
| **GitHub Packages** | 500 MB 儲存<br>1 GB 傳輸 / 月 | 1 GB 儲存<br>無限傳輸 / 月 |
| **儲存庫容量** | 建議 < 5GB<br>(硬上限 100GB) | 建議 < 5GB<br>(硬上限 100GB) |

### 共用額度說明 (Shared Quota Notice)

請特別注意，GitHub Actions, GitHub Packages 的免費額度，都是以整個 GitHub 組織 (Organization) 為單位進行計算的，而非為單一儲存庫提供獨立額度。任一專案的用量增加，都會消耗所有專案可用的共同額度。

### 用量管理與請求 (Usage Management & Requests)

為確保共用額度的有效利用，請開發夥伴務必在自有開發環境中充分測試，確認交付項目無誤後再進行版本推送，以減少不必要的 CI/CD 資源消耗。若因專案緊急需求必須進行頻繁的推送與部署，請務必提前聯繫我方資訊承辦單位進行評估，避免影響整體組織的 CI/CD 運作。

## 3. 資通安全規範

> **此為最高優先級規範**

所有在本儲存庫的協作行為，皆須嚴格遵守國家表演藝術中心國家兩廳院資通安全管理制度。
